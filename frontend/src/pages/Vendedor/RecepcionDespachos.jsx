...
export default function RecepcionDespachos() {
  ...
  const handleRecepcionar = async (record) => {
    ...
    // if (record.ordencompra.estado === "recibida con faltante") {
    //   buscarSucursales();
    //   try {
    //     const respuesta = await obtenerOrdenCompraProveedorPorNombre(
    //       record.ordencompra.nombreOrden,
    //     );
    //     console.log("respuea de obtener orden por nombre", respuesta.data);
    //     if (respuesta.status === 200) {
    //       setOrdenSeleccionada(respuesta.data);
    //       formRecepcion.resetFields();

    //       const detallesDespacho =
    //         respuesta.data?.ordencompra?.despacho?.detalledespachos || [];

    //       let detallesIniciales =
    //         respuesta.data?.ordencompra?.compraproveedordetalles?.map(
    //           (detalle, index) => {
    //             const detalleDespacho = detallesDespacho[index] || {};
    ...
    //               cantidadRechazada:
    //                 Number(
    //                   detalleDespacho?.cantidadRechazada ??
    //                     detalleDespacho?.cantidad ??
    //                     0,
    //                 ) || 0,
    //             };
    ...
    //     notification.error({
    //       message: "Error",
    //       description: respuesta.error || "Error al obtener la orden de compra",
    //     });
    //   } catch (error) {
    ...
    buscarSucursales();
    setOrdenSeleccionada(record);
    console.log("orden recuperada", record);
    formRecepcion.resetFields();
  ...
  return (
    <div>
      ...
      >
        ...
        {/**Card con datos de los despachos */}
        {ordenDrawer?.despachos?.length > 0 ? (
          ...
          <div style={{ paddingTop: 20 }}>
            <Card title="Despachos" size="small">
              <p>
                Sin Despachos para la orden {ordenDrawer?.nombreOrden || "-"}
              </p>
            </Card>
          </div>
        )}
